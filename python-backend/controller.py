        ...
        while True:
            ...
            try:
                ...
                await manager.send_json_message(
                    {"type": "error_from_python", "message": "Invalid message format received."}, websocket)
                continue
            ...
            if message_type == "transcription_frontend" and isinstance(text_content, str):
                ...
                if normalized_text.startswith(trigger_word_backend):
                    ...
                    print(f"Potential command phrase: '{potential_command_phrase}'")

                    extracted_city = potential_command_phrase

                    if extracted_city:
                        print(f"City extracted: '{extracted_city}'. Fetching weather...")
                        ...
                        print(
                            f"WeatherExtractor found no city in: '{potential_command_phrase}' (Original: '{text_content}')")
                        await manager.send_json_message({
                    ...
                    print(
                        f"Trigger word '{trigger_word_backend}' not found at start of: '{normalized_text}'. Ignoring.")
                ...
                await manager.send_json_message(
                    {"type": "error_from_python", "message": "Unbekannter Nachrichtentyp oder fehlender Text."},
                    websocket)
            else:  # Weder Typ noch Text
                print(f"Received invalid/empty message structure from {websocket.client}: {data}")
                await manager.send_json_message(
                    {"type": "error_from_python", "message": "Ungültige Nachricht erhalten."}, websocket)
